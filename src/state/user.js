...
export const createUser = createAsyncThunk("CREATE_USER", (user) => {
  ...
  return axios
    .post(`http://${ip}:3001/api/user/register`, user)
    .then((res) => res.data)
    .catch((error) =>
...
export const loginUser = createAsyncThunk("LOGIN_USER", (user) => {
  ...
  return axios
    .post(`http://${ip}:3001/api/user/login`, user)
    .then((res) => {
...
export const addPlan = createAsyncThunk("ADD_PLAN", (plan) => {
  ...
  return AsyncStorage.getItem("token")
    .then((token) => {
      return axios.post(`http://${ip}:3001/api/user/planToAttend`, plan, {
...
export const removePlan = createAsyncThunk("REMOVE_PLAN", (plan) => {
  ...
  return AsyncStorage.getItem("token")
    .then((token) => {
      ...
      return axios.delete(`http://${ip}:3001/api/user/deletePlan/${id}`, {
